...
_ServiceStack.Redis_ - I am a long time user of this library. What I don't like about it are the command function names. First of all, the fact that they are different from the actual Redis commands. The problem is, I can never remember what the Redis commands are when I'm working with a different client, in particular the CLI. Also, consider AddItemToList, EnqueueItemOnList and PushItemToList - they all do the same thing. Why the duplication? Why isn't RPUSH a better name than all three of them (when I look through the list, I regularly wonder whether items are placed at the front or end of the list).
