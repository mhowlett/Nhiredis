...
Nhiredis provides an interface that closer matches the raw Redis commands than the two .NET clients recommended on redis.io - ServiceStack.Redis and BookSleeve. This in my opinion makes it's usage more readable. But Nhiredis is not lower level - the type coercing functionality is very convenient and in my experience, using it is more convenient than the competition. 
